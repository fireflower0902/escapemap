"""
리멤버방탈출(rememberesc.com) 테마 + 스케줄 DB 동기화 스크립트.

사이트: http://rememberesc.com
플랫폼: Node.js/Express 자체 API (reverseesc.com과 동일 엔진)

지점:
  수원점  cafe_id=475592733   area=gyeonggi  (경기 수원시 팔달구)
  천안점  cafe_id=49858990    area=cheonan   (충남 천안시 동남구)
  부평점  cafe_id 미정        area=incheon   (카카오 미등록)

API:
  GET http://rememberesc.com/service/get_data
  → 인증 없음, 전체 지점 데이터 한번에 반환
  응답:
    theme_data: [{
      _id, theme_name, difficulty, degree_fear, esti_time, price,
...
요일 분류:
  평일   = 월·화·수·목 (weekday 0,1,2,3)
  금요일 = 금 (weekday 4)
  주말   = 토·일 (weekday 5,6)

실행:
  cd escape-aggregator/backend
  uv run python scripts/sync_rememberesc_db.py
  uv run python scripts/sync_rememberesc_db.py --no-schedule
  uv run python scripts/sync_rememberesc_db.py --days 14
"""
...
LOCATION_MAP = {
    ...
    # 부평점은 카카오 데이터 미등록 → 추후 추가 가능
}

# 요일 번호(0=월) → week_name 매핑
_WEEKDAY_MAP = {
    0: "평일",   # 월
    1: "평일",   # 화
    2: "평일",   # 수
...
def sync_cafe_meta(db) -> None:
    for loc, info in LOCATION_MAP.items():
        ...
        print(f"  [UPSERT] 카페: 리멤버방탈출 {info['branch_name']} (id={info['cafe_id']})")
...
def main(run_schedule: bool = True, days: int = 14) -> None:
    print("=" * 60)
    print("리멤버방탈출(rememberesc.com) → DB 동기화")
    print("=" * 60)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="리멤버방탈출 DB 동기화")
