...
지점:
  강남1호점    location_id=2  cafe_id=1030963843  area=gangnam
  대구동성로점  location_id=3  cafe_id=1443757324  area=daegu
...
BRANCHES = [
    {
        ...
        "location_id":  2,
        "branch_name":  "강남1호점",
        "address":      "서울시 강남구 강남대로 96길 19, 4층",
        "area":         "gangnam",
    },
    {
        "cafe_id":      "1443757324",
...
def _fetch_calendar(year: int, month: int, location_id: int) -> dict[int, list[dict]]:
    """
    월별 캘린더 조회.
    반환: {day: [meeting, ...]}
    """
...
def sync_theme(cafe_id: str, title: str, poster_url: str | None) -> str | None:
...
def sync_schedules(cafe_id: str, slots: list[dict], theme_id_map: dict[str, str]):
...
def sync_one_branch(branch: dict, months: int, run_schedule: bool, months_to_fetch: list) -> None:
    ...
    print(f"  [UPSERT] 카페: 마피아카페 {branch_name} (id={cafe_id})")

    # 캘린더 수집
    all_slots: list[dict] = []
    for year, month in months_to_fetch:
        print(f"  {year}년 {month}월...")
        cal = _fetch_calendar(year, month, location_id)
    ...
    for title, poster_url in theme_info.items():
        ...
        doc_id = sync_theme(cafe_id, title, poster_url)
        if doc_id:
    ...
    if run_schedule:
        ...
        for d in sorted(by_date):
            print(f"  {d}: 가능 {by_date[d]['available']}개 / 마감 {by_date[d]['full']}개")
        sync_schedules(cafe_id, all_slots, theme_id_map)
...
def main(run_schedule: bool = True, months: int = 2):
    ...
    for branch in BRANCHES:
        ...
        try:
            sync_one_branch(branch, months, run_schedule, months_to_fetch)
        except Exception as e:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="마피아카페 DB 동기화")
